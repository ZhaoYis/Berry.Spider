...
public partial class MainWindowViewModel : ViewModelBase, IRecipient<NotificationTaskMessage>, ITransientDependency
{
    ...
    /// 应用名称
    /// </summary>
    [ObservableProperty] private string _applicationTitle = "Berry.Spider.ToolkitStore";

    /// <summary>
    /// 主窗口提示信息
    /// </summary>
    [ObservableProperty] private string _tips = "Berry.Spider.ToolkitStore";
    ...
    public void Receive(NotificationTaskMessage message)
    {
        switch (message.IsRunning)
        {
            case true:
            {
                _timer.Elapsed += (sender, e) => { this.Tips = $"正在处理请稍后。当前已执行{_stopwatch.ElapsedMilliseconds / 1000}秒..."; };
                _stopwatch.Start();
                _timer.Start();
                break;
            }
