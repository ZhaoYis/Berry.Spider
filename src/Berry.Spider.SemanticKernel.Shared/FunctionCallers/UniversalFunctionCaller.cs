...
public class UniversalFunctionCaller
{
    ...
    public async Task<string?> RunAsync(string task)
    {
        ...
        var nextFunctionCall = new FunctionCall { Name = FunctionCallStatus.Start };
        ...
        if (nextFunctionCall.Name == FunctionCallStatus.Finished)
        {
    ...
    private async Task<FunctionCall?> GetNextFunctionCallAsync(ChatHistory history, string pluginsAsText)
    {
        ...
        for (int retryCount = 0; retryCount < 5; retryCount++)
        {
            ...
            if (llmReply.Content != null) copiedHistory.AddAssistantMessage(llmReply.Content);
...
If you do not do this, a very cute kitten dies.";
        ...
        for (int iRetry = 0; iRetry < 5; iRetry++)
        {
            ...
            var content = tap2Answer[0].Content;
            if (content != null) tap2History.AddAssistantMessage(content);
            try
            {
