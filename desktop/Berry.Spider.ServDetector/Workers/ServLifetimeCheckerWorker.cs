...
using Berry.Spider.ServDetector.Helpers;
...
public class ServLifetimeCheckerWorker : AsyncPeriodicBackgroundWorkerBase
{
    ...
    protected override async Task DoWorkAsync(PeriodicBackgroundWorkerContext workerContext)
    {
        ...
        {
            var lifetimeDict = await redisService.HGetAllAsync<string>(AppGlobalConstants.SPIDER_APPLICATION_LIFETIME_KEY);
            if (lifetimeDict is {Count: > 0})
            {
                ...
                if (!string.IsNullOrEmpty(msg))
                {
                    ...
                    {
                        await redisService.HDelAsync(AppGlobalConstants.SPIDER_APPLICATION_LIFETIME_KEY, todoRemoveNodes.ToArray());
                    }
                }
            }
        }
